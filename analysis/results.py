...
def fig_capex_pie(result):
    ...
    fixed = {
        ...
        "Steam turbine system": df.loc[df.item == "steam_turbine_system", "cost_usd"].sum(),
    ...
    for at in autotexts:
        at.set_fontsize(8)
    ax.set_title(f"CAPEX build-up — ${result['capex']['total_capex_usd']/1e6:,.1f}M total")
...
def fig_cashflows(result):
    ...
    ax.bar(years, -(cf["opex_usd"] + cf["tax_usd"]) / 1e6,
           color="#C44E52", alpha=0.55, label="Tax")
...
def fig_dispatch_profile(result):
    ...
    ax.fill_between(h, t["export_mw"][sl], color="#DD8452", alpha=0.35, label="Export to grid")
...
def write_report(result):
    ...
    ap("")
    ap(f"*Scenario: `{result['scenario']}` · dispatch: `{result['dispatch_mode']}` · horizon {result['horizon']} yrs*")
    ap("")
    ap("## Headline returns (nominal USD)")
    ap("")
    ap("| Metric | Value |")
    ap("|---|---|")
    ap(f"| CAPEX | {fmt_m(m['capex_usd'])} ({m['capex_usd']/result['capex']['net_mw']/1e3:,.0f} USD/kW net) |")
    ...
    ap(f"| Imported MWh | {e.loc[1,'import_mwh']:,.0f} | {e.loc[6,'import_mwh']:,.0f} | {e.loc[15,'import_mwh']:,.0f} |")
    ap(f"| Fuel burn (MMBtu) | {e.loc[1,'fuel_mmbtu']:,.0f} | {e.loc[6,'fuel_mmbtu']:,.0f} | {e.loc[15,'fuel_mmbtu']:,.0f} |")
    ap(f"| EBITDA (USD) | {fmt_m(rev.loc[1,'total_revenue_usd']-opx.loc[1,'total_opex_usd'])} | {fmt_m(rev.loc[6,'total_revenue_usd']-opx.loc[6,'total_opex_usd'])} | {fmt_m(rev.loc[15,'total_revenue_usd']-opx.loc[15,'total_opex_usd'])} |")
    ap(f"| Plant availability | {e.loc[1,'availability']:.3f} | {e.loc[6,'availability']:.3f} | {e.loc[15,'availability']:.3f} |")
    ap("")
    ap("## Narrative")
    ap("")
    ap("- Gross 20.06 MWe / net 19.6 MWe CCGT; fuel at delivered CNG; max-export dispatch.")
    ap("- Early years (P1, 10 MW DC) operate at ~78% capacity factor and export ~80 GWh/yr to the TCN grid; revenue from exports is a core early stream.")
    ap("- At P3 (20 MW DC, PUE 1.20) peak demand 21 MW exceeds net 19.6 MW capacity; outages and the tail of the load curve cause modest grid purchases (2-4% of demand).")
    ap("- 2x1 reliability: plant availability ≈ 97.8% (single-GT and full-plant outage windows covered by grid import, not customer outage).")
    ap("- Financial assumptions flagged WORKING (gas price, PPA/export tariffs, CAPEX, financing) — see sensitivity analysis for swings.")
    ap("")
    ap("*Generated by `analysis/results.py`. All figures: `outputs/figures/`. All tables: `outputs/tables/`.*")
...
def main(argv=None):
    ...
    result = run_project(cfg, dispatch_mode=args.dispatch)
    ...
    result["energy_df"].reset_index().to_csv(OUT / "tables" / "annual_energy.csv", index=False)
    result["revenue_df"].reset_index().to_csv(OUT / "tables" / "annual_revenue.csv", index=False)
    result["opex_df"].reset_index().to_csv(OUT / "tables" / "annual_opex.csv", index=False)
    result["capex"]["register_df"].to_csv(OUT / "tables" / "capex_register.csv", index=False)
    pd.DataFrame([result["metrics"]]).to_csv(OUT / "tables" / "financial_summary.csv", index=False)
    ...
    m = result["metrics"]
    print("Baseline OK: project NPV=%s IRR=%.1f%% equity IRR=%.1f%% LCOE=$%.2f/MWh"
          % (fmt_m(m["project_npv_usd"]), m["project_irr_pct"], m["equity_irr_pct"], m["lcoe_usd_per_mwh"]))
